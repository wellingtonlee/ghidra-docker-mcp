...
- `src/ghidra_mcp/__main__.py` — CLI entry point with `--transport`, `--host`, `--port` flags
...
- `create_server()` accepts `host`/`port` kwargs, passed through to `FastMCP()`
- CLI supports `--transport sse` for HTTP transport (requires `pip install -e ".[sse]"`)
